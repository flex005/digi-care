...
describe('the name column', () => {
  ...
  /**
   * The guard on a derived surname. `listName` takes the last token of the
   * legal name, which is right for every fixture name and wrong for the first
   * one with a particle — "Anna van der Berg" would render "Anna Berg".
   *
   * When this fails, the answer is a real surname field on `Resident`, not a
   * cleverer split. It is here to force that decision rather than let somebody
   * quietly ship a mangled name.
   */
  it('fails when a legal name stops being two tokens', () => {
    for (const resident of residents) {
      expect(
...
describe('the tiles do not speak before the data arrives', () => {
  /**
   * "We have not looked yet" and "we looked and there is nobody" are opposites,
   * and an aggregate over an unloaded list renders as the second. Loading must
   * not produce "Insufficient evidence — there are no residents here to count".
   */
  it.each([
    ['loading', '/residents?sim=loading'],
    ['error', '/residents?sim=error'],
  ])('renders no tiles in the %s state', async (_label, entry) => {
    renderList(entry)
  ...
  it('does render them once an empty site has actually loaded', async () => {
    renderList('/residents?sim=empty')
